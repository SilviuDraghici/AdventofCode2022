...
class Node:
  ...
  def calcSize(self, candidate_dirs, max_size):
    if(self.size == 0):
      size = 0
      for node in self.children.values():
        size += node.calcSize(candidate_dirs, max_size)
      if size <= max_size:
...
def soln():
  ...
  candidate_dirs = []
  root.calcSize(candidate_dirs, max_size)
  print("The total file size is {}".format(sum(candidate_dirs)))  
