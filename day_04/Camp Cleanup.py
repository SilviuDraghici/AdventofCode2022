...
if __name__ == "__main__":
  print("Finding assignment overlaps...")
 
  fileName = 'camp_assignments.txt'
  ...
  while line:
    ...
    range_1 = list(map(int, range_1))
    
    range_2 = ranges[1].split("-")
    range_2 = list(map(int, range_2))
    
    if((range_1[0] <= range_2[0] and range_2[1] <= range_1[1]) or
       (range_2[0] <= range_1[0] and range_1[1] <= range_2[1])):
      num_overlaps += 1
  ...
  print("There are {} overlaps".format(num_overlaps))