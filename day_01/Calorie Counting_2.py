...
if __name__ == "__main__":
  print("Finding number of calories held by the top 3 elves...")
  ...
  while True:
    ...
    if line == "":
      if(current_calories > max_calories[0]):
        heapq.heapreplace(max_calories, current_calories)
      current_calories = 0
  ...
  # check if last elf is in top 3
  if(current_calories > max_calories[0]):
    heapq.heapreplace(max_calories, current_calories)
        
  
  print("The top three elves are carrying {} calories!".format(max_calories))
  print("The total of the top three is: {}".format(sum(max_calories)))
