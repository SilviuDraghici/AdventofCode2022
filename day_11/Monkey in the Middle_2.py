...
class Monkey:
  def __init__(self):
    ...
    self.val1 = lambda a: a
    self.op = lambda a, b: a
    self.val2 = lambda a: a
  ...
  def keep_away(self):
    while len(self.items) > 0:
      ...
      val = self.op(self.val1(val), self.val2(val))
      val  = val % supermod
      if val % self.divis == 0:
        monkeys[self.true_pass].items.append(val)
      else:
        monkeys[self.false_pass].items.append(val)
...
if __name__ == "__main__":
  ...
  while line:
    ...
    line = inputFile.readline().strip()[16:]
    line = line.split(", ")
    for num in line:
      monkey.items.append(int(num))
    
    line = inputFile.readline().strip()[17:]
    line = line.split(" ")
    ...
    match line[1]:
      ...
      case '-':
        monkey.op = operator.sub
      case '*':
        monkey.op = operator.mul
      case '/':
        monkey.op = operator.div
    ...
    match line[2]:
      case "old":
        ...
        monkey.val2 = val
      case _:
        val = getLiteral(int(line[2]))
        monkey.val2 = val
    
    line = inputFile.readline().strip().split(" ")
    monkey.divis = int(line[3])
    
    line = inputFile.readline().strip().split(" ")
    monkey.true_pass = int(line[5])
    
    line = inputFile.readline().strip().split(" ")
    monkey.false_pass = int(line[5])    
    ...
    val1 = monkey.val1(5)    
    val2 = monkey.val2(5)
    val = monkey.op(monkey.val1(5), monkey.val2(5))
  ...
  for monkey in monkeys:
    supermod *= monkey.divis
  
  for _ in range(10000):
    if _ % 100 == 0:
      print(f"{_} {[len(monkey.items) for monkey in monkeys]}")
    for monkey in monkeys:
      monkey.keep_away()
  
  monkeys.sort(key=lambda monkey: monkey.inspects, reverse = True)

  print(f"{monkeys[0].inspects * monkeys[1].inspects}")
