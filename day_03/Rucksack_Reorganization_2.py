...
if __name__ == "__main__":
  print("Finding rucksack group badges...")
 
  fileName = 'rucksack_inventories.txt'
  ...
  while line:
    ...
    set_1 = set(line)
    set_2 = set(inputFile.readline().strip())
    set_3 = set(inputFile.readline().strip())
    
    
    group_badge = list(set_1 & set_2 & set_3)[0]
    badge_val = ord(group_badge) - 97 + 1 if ord(group_badge) >= 97 else ord(group_badge) - 65 + 27
    group_badge_sum += badge_val
    
    line = inputFile.readline()
  inputFile.close()

  print("The group badge priority sum is {}!".format(group_badge_sum))